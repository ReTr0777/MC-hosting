...
export async function prepareTerrariaServerDir(serverDir: string, spec: GameServerSpec): Promise<void> {
  ...
  /*
   * tModLoader needs both the folder and the file to exist before its first boot.
   * A missing `enabled.json` means no mod loads at all — including on a server whose
   * mods were restored from a backup — and the server says nothing about why.
   */
  if (cfg.variant === 'TMODLOADER') {
    fs.mkdirSync(modsDir(serverDir), { recursive: true });
    if (!fs.existsSync(enabledJsonPath(serverDir))) writeEnabledMods(serverDir, []);
  }
...
export const terraria: GameDefinition = {
  ...
  async prepareWorld(serverDir, binaryPath, spec, log) {
    ...
    /*
     * Worlds are always generated by the **vanilla** binary, even for a tModLoader
     * server.
     *
     * Generation is driven by typing into Terraria's interactive menus, and those
     * prompts are the one part of this that had to be discovered against a live
     * server rather than read from documentation. tModLoader shows the same menus
     * behind several seconds of mod-loading output and a .NET bootstrap, so putting
     * it in this path would mean re-deriving the whole sequence for a second binary
     * to produce the identical file.
     *
     * A vanilla world is exactly what tModLoader expects: converting one is the
    ...
    await generateTerrariaWorld(serverDir, worldgenBinary, spec, log);
    log('World generated.');
  },
  ...
  /*
   * Everything Terraria owns lives under `worlds/`; the binary stays in the shared
   * cache and is never inside the server directory.
   *
   * `Mods` is here too, and has to be: a restored backup carries the `.tmod` files
   * and the `enabled.json` that the restored world expects. Leaving the old Mods
   * folder in place would load the previous server's mods into the restored world,
   * which for a modded world is data corruption rather than a wrong setting.
   */
  restoreClearDirs: ['worlds', 'Mods'],
