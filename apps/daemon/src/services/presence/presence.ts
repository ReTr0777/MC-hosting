...
import { getContainerByIdOrName } from '../runtime/docker';
import { processManager } from '../runtime/process';
import { getGame, gameOfServer } from '../../games';
...
class PresenceService extends EventEmitter {
  ...
  public ingestLine(serverId: string, line: string): void {
    ...
    // Another game words its join and leave lines differently, so its own module
    // parses them. Everything below this branch is the original Minecraft body,
    // which Minecraft still reaches unchanged because it has no module.
    const definition = getGame(gameOfServer(serverId));
    if (definition) {
      const event = definition.parsePresenceLine(line);
      if (!event) return;
      if (event.type === 'join') {
        // Same rule as Minecraft's: a duplicate join without a leave keeps the
        // earlier start rather than resetting the clock.
        if (!state.online.has(event.username)) {
          state.online.set(event.username, { username: event.username, joinedAt: new Date() });
          this.emit('join', { serverId, username: event.username });
        }
      } else {
        this.closeSession(serverId, event.username);
      }
      return;
    }
