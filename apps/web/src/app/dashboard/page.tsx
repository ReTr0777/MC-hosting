...
function TerrariaWizardStep({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      {/* Version. Separate from the variant because the two lists are unrelated: a
          tModLoader build is dated, a Terraria build is numbered, and only one of them
          applies at a time. */}
      <div>
        <label style={{ display: 'block', fontSize: '0.72rem', color: 'var(--text-muted)', marginBottom: '5px', fontWeight: 600 }}>
          {config.variant === 'TMODLOADER' ? 'tModLoader build' : 'Terraria version'}
        </label>
        {config.variant === 'TMODLOADER' ? (
          <>
            <select
              className="cc-input"
              value={config.tmodloaderVersion ?? DEFAULT_TMODLOADER_VERSION}
              onChange={(e) => onChange('tmodloaderVersion', e.target.value)}
            >
            ...
            <p className="cc-help">
              tModLoader tracks an older Terraria than vanilla ships, so the world is generated with the version
              this build expects rather than the newest one. Mods are compiled against a specific build and will
              not load on another, so changing this later means replacing them.
            </p>
          </>
        ) : (
          <>
            <select
              className="cc-input"
              value={config.terrariaVersion ?? DEFAULT_TERRARIA_VERSION}
              onChange={(e) => onChange('terrariaVersion', e.target.value)}
            >
              {TERRARIA_VERSIONS.map((v) => (
                <option key={v} value={v}>{v}{v === DEFAULT_TERRARIA_VERSION ? ' (recommended)' : ''}</option>
              ))}
            </select>
            <p className="cc-help">
              The world is generated by this version and stays in its format, so pick it before creating the server.
            </p>
          </>
        )}
      </div>
