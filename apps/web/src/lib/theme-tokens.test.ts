...
test('the shipped Cherry Blossom file parses cleanly', () => {
  const file = fs.readFileSync(path.join(process.cwd(), 'themes/cherry-blossom.css'), 'utf8');
  ...
  assert.equal(theme.name, 'Cherry Blossom');
  assert.equal(theme.scheme, 'light');
  // It is a complete theme, so nothing should need filling in.
  assert.equal(Object.keys(theme.tokens).length, COLOR_TOKENS.length);
...
test('the shipped Sakura Drift file parses cleanly and keeps its petals', () => {
  const file = fs.readFileSync(path.join(process.cwd(), 'themes/sakura-drift.css'), 'utf8');
  const { theme, errors, warnings } = parseThemeFile(file);

  assert.deepEqual(errors, []);
  assert.deepEqual(warnings, []);
  assert.ok(theme);
  assert.equal(theme.name, 'Sakura Drift');
  assert.equal(theme.scheme, 'light');
  assert.equal(theme.tokens['--bg-animation'], 'fall');
  assert.equal(theme.tokens['--bg-size'], '240px');
