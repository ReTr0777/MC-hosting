...
function health(memoryTotal: number, allowance?: { memoryMb: number; cpus: number; capped: boolean }) {
  return {
...
test('an uncapped node reports its hardware, not the allowance echoing it back', () => {
  ...
  const patch = reportedCapacityPatch(health(32768, { memoryMb: 32768, cpus: 16, capped: false }));
...
test('a capped node reports what it offers, not what it has', () => {
  ...
  const patch = reportedCapacityPatch(health(32768, { memoryMb: 8192, cpus: 4, capped: true }));
  assert.deepEqual(patch, { totalMemory: 8192, totalCpu: 4 });
});

test('a fractional CPU allowance rounds down, never handing back a thread held in reserve', () => {
  const patch = reportedCapacityPatch(health(16384, { memoryMb: 4096, cpus: 2.5, capped: true }));
  assert.equal(patch.totalCpu, 2);
});

test('an allowance below one CPU still leaves the node able to hold something', () => {
  // floor(0.5) is 0, and a node registered with zero CPUs can never be placed on —
  // it would drop out of the scheduler entirely rather than take small servers.
  const patch = reportedCapacityPatch(health(16384, { memoryMb: 2048, cpus: 0.5, capped: true }));
