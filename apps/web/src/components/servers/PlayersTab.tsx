...
import { Game, GAME_CAPABILITIES } from '@mc-manager/shared';
...
export default function PlayersTab({
  ...
  canManage = true,
  game = Game.MINECRAFT,
}: { serverId: string; canManage?: boolean; game?: Game }) {
  // Terraria has no operator system, and its ban list is a flat file the panel does
  // not manage — so the buttons it cannot honour are hidden rather than shown and
  // silently doing nothing.
  const caps = GAME_CAPABILITIES[game];
      ...
      const ok = await confirm({
        ...
        message:
          action === 'ban'
            ...
            : `${username} will be disconnected and blocked from rejoining. Unbanning is done by editing banlist.txt under Files.`
